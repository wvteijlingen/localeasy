...
// function findIndexOfColumn(name: string, headerRow: Array<string | undefined>): number {
//   const index = headerRow.indexOf(name)
//   if (index === -1) {
//     throw new UserError(`Column "${name}" does not exist in the spreadsheet. Please add a column named "${name}".`)
//   }
//   return index
// }

// function getIndexes(locales: string[], headerRow: Array<string | undefined>): ColumnIndexes {
//   const translationIndexes = Object.fromEntries(
//     locales.map(locale => {
//       const index = findIndexOfColumn(locale, headerRow)
//       return [locale, index]
//     })
//   )

//   return {
//     keyIndex: findIndexOfColumn("key", headerRow),
//     commentIndex: findIndexOfColumn("comment", headerRow),
//     translationIndexes,
//   }
// }
//
...
//   const values = await sheets.spreadsheets.values.get({ spreadsheetId: sheetID, range: sheetName + "!A1:Z" })
...
//   return extractLocalizations(rows, locales)
// }

// export function extractLocalizations(rows: string[][], locales: string[]): MultiTranslation[] {
//   const indexes = getIndexes(locales, rows[0])

//   const localizations = rows.slice(1).map((row, index) => {
//     const rowNumber = index + 2 // Offset to take the sheet header into account.
//     const key = row[indexes.keyIndex]

//     if(key === undefined || key === "") {
//       throw new UserError(`Invalid sheet: Missing key for entry at row ${rowNumber}.`)
//     }

//     const translations: Array<[string, string]> = locales.map(locale => {
//       const translation = row[indexes.translationIndexes[locale]]
//       if(translation === undefined || translation === "") {
//         throw new UserError(`Invalid sheet: Missing '${locale}' translation for entry '${key}' at row ${rowNumber}.`)
//       }
//       return [locale, translation]
//     })
...
//     return {
//       key: key,
//       comment: row[indexes.commentIndex],
//       translations: Object.fromEntries(translations)
//     }
//   })
